...
export class EventDetailComponent implements OnInit {
  ...
  GetEventdetails() {
    this.dataService.GetAllEventDetails().subscribe(
      (result: any) => {
        this.showLoader = true;
        const loginResult = result as IResultVM;
        if (loginResult.status === StatusEnum.Success) {
          this.eventdetailsDataSource = new MatTableDataSource<IEventDetail>(
            loginResult.data as IEventDetail[]
          );
  ...
  onDeleteEventdetails(eventdetaildata: IEventDetail) {
    ...
    dialogRef.afterClosed().subscribe((dialogresult: IDialogMessage) => {
      if (dialogresult.valid === true) {
        this.dataService.DeleteEventDetail(dialogresult.data.id).subscribe(
          (result: any) => {
            this.showLoader = true;
            ...
            if (eventdetailResult.status === StatusEnum.Success) {
              ...
              this.showToast(
                "success",
                "Event Success",
                "Deleted " + eventdetail.event_detail_name + " Successfuly"
              );
  ...
  OnCreateEventdetails() {
    ...
    dialogRef.afterClosed().subscribe((dialogresult: IDialogMessage) => {
      if (dialogresult.valid === true) {
        this.dataService.CreateEventDetail(dialogresult.data).subscribe(
          (result: any) => {
            this.showLoader = true;
            const eventdetailResult = result as IResultVM;
            const eventdetail = eventdetailResult.data as IEventDetail;
  ...
  onViewEventdetails(eventdetaildata: IEventDetail) {
    ...
    dialogRef.afterClosed().subscribe((dialogresult: IDialogMessage) => {
      if (dialogresult.valid === true) {
        this.dataService.UpdateEventDetail(dialogresult.data).subscribe(
          (result: any) => {
            this.showLoader = true;
            const eventdetailResult = result as IResultVM;
            const eventdetail = eventdetailResult.data as IEventDetail;
