...
export class UserDataService {
  ...
  register(uservm: IRegisterVM): Observable<IResultVM> {
    ...
    return this.http
      .post(this._baseUrl + "user/create/", JSON.stringify(uservm), {
        headers: headers
      })
  ...
  login(creds: ILoginVM): Observable<LoginResponseVM> {
    ...
    headers.append("Content-Type", "application/json");
    return this.http
      .post(this._baseUrl + "user/token/", JSON.stringify(creds), {
        headers: headers
      })
      .map((res: Response) => res.json())
      .catch(this.handleError);
  ...
  logout(creds: IUser): Observable<IUser> {
    return this.http
      .post(this._baseUrl + "user/SignOut", JSON.stringify(creds), {})
      .map((res: Response) => res.json())
      .catch(this.handleError);
  }
